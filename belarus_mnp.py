...
def parse_file(file_in, base_settings):
    try:
        ...
        with open(base_settings.bel_conf.lock_file, 'w') as lock_mnp_db:
...
def delete_tmp_files(settings: utils.BaseSettings, *args):
    for file in args:
        if isinstance(file, Path):
            if file.exists():
                os.remove(file)
    if settings.bel_conf.lock_file.exists():
        os.remove(settings.bel_conf.lock_file)
...
def processing_mnp(base_settings: utils.BaseSettings):
    ...
    source_file_mask = base_settings.bel_conf.source_dir.joinpath(
        base_settings.bel_conf.source_file_mask)
